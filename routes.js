...
(function(){
    ...
    ensureSdk(function(ok){
      ...
      scopeCenter(function(center){
        ...
        rawTextSearch(req, function(res, st){
          ...
          if (st !== OK || !res || !res.length) return cb([]);
          cb(shape(q, res, sc));
        });
      });
    });
  ...
  function shape(q, res, sc){
    ...
    // 都道府県が選ばれていれば、その県のものを先に並べる
    if (sc && sc.kind === 'pref' && sc.pref){
      var inPref = list.filter(function(x){ return x.pref === sc.pref; });
      if (inPref.length) list = inPref.concat(list.filter(function(x){ return x.pref !== sc.pref; }));
    }
    return list;
  }
  ...
  /* ── 同名候補を選ぶシート ───────────────────────────────── */
  function openPicker(q, list, done){
    open('<div class="wpk-hd"><div class="wpk-ttl">どちらの「' + esc(q) + '」ですか？</div>'
      + '<div class="wpk-sub">同じ名前の神社・お寺が ' + list.length + ' か所見つかりました。<br>目的の場所をお選びください。</div></div>'
      + '<div class="wpk-list">'
  ...
  function insertField(){
    ...
    if (IS_PC){
      ...
      btn.type = 'button';
      row.appendChild(btn);
      fields.parentNode.insertBefore(label, fields.nextSibling);
  ...
  function wrapStart(name){
    ...
    var wrapped = function(){
      ...
      // Places が遅い・使えないときも必ず動くように、6秒で従来動作へ
      var guard = setTimeout(go, 6000);

      findCandidates(q, function(list){
        clearTimeout(guard);
        if (list === null) return go();                         // Placesが使えない → 従来どおり
        if (!list.length){ toast('「' + q + '」の検索結果が見つかりませんでした'); return go(); }
